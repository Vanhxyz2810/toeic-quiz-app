...
import { groups, parts, resolveQuestionSet, tests } from "@/lib/data";
...
export function Dashboard() {
  ...
  // Tiền tố set key theo đề đang chọn ("" = Đề 1, "t2-" = Đề 2).
  const tp = test === "test2" ? "t2-" : "";
  ...
  // Nhóm thuộc đề đang chọn, lọc thêm theo search (số câu hoặc keyword).
  const filteredGroups = useMemo(() => {
    const inTest = groups.filter((g) => g.test === test);
    const q = query.trim().toLowerCase();
    if (!q) return inTest;
    return inTest.filter((g) => {
    ...
    <div className="space-y-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Luyện TOEIC Part 3 → 7 · 2 đề practice test — chọn đề và chế độ bên dưới
                  để bắt đầu. Tiến độ được lưu tự động trên trình duyệt.
                </p>
              </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <QuickStart href={`/quiz/${tp}all`} icon={BookOpen} title="Toàn bộ đề" subtitle="Tất cả câu Part 3 → 7" count={resolveQuestionSet(`${tp}all`).length} mode={mode} query={qs} />
          <QuickStart href={`/quiz/${tp}part-3`} icon={Layers} title="Part 3" subtitle="Conversations" count={resolveQuestionSet(`${tp}part-3`).length} mode={mode} query={qs} />
          <QuickStart href={`/quiz/${tp}part-4`} icon={Layers} title="Part 4" subtitle="Short talks" count={resolveQuestionSet(`${tp}part-4`).length} mode={mode} query={qs} tone="success" />
          <QuickStart href={`/quiz/${tp}part-5`} icon={PenLine} title="Part 5" subtitle="Incomplete sentences" count={resolveQuestionSet(`${tp}part-5`).length} mode={mode} query={qs} />
          <QuickStart href={`/quiz/${tp}part-6`} icon={FileText} title="Part 6" subtitle="Text completion" count={resolveQuestionSet(`${tp}part-6`).length} mode={mode} query={qs} tone="success" />
          <QuickStart href={`/quiz/${tp}part-7`} icon={BookOpenText} title="Part 7" subtitle="Reading comprehension" count={resolveQuestionSet(`${tp}part-7`).length} mode={mode} query={qs} />
          <QuickStart href={`/quiz/${tp}graphic`} icon={ImageIcon} title="Câu có graphic" subtitle="Bảng biểu & sơ đồ (Part 3–4)" count={resolveQuestionSet(`${tp}graphic`).length} mode={mode} query={qs} tone="amber" />
          <QuickStart href="/quiz/wrong" icon={XCircle} title="Ôn câu sai" subtitle="Câu sai của cả 2 đề" count={wrongCount} mode={mode} query={qs} tone="danger" disabled={wrongCount === 0} />
          <QuickStart href="/quiz/bookmarked" icon={Bookmark} title="Câu đã đánh dấu" subtitle="Bookmark của cả 2 đề" count={bookmarkCount} mode={mode} query={qs} tone="primary" disabled={bookmarkCount === 0} />
        </div>
      </section>
            ...
            <div key={part} className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {cards.map((c) => {
                  ...
                  const answered = c.questions.filter((q) => progress.answers[q.uid]).length;
                  const correct = c.questions.filter(
                    (q) => progress.answers[q.uid] === q.correctAnswer
                  ).length;
